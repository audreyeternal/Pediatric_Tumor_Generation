...
class Visualizer():
    def __init__(self, opt):
        ...
        if self.tf_log:
            ...
            with self.tf.compat.v1.Graph().as_default():
    ...
    def display_current_results(self, visuals, epoch, step):
        ...
        if self.tf_log: # show images in tensorboard output
            ...
            for label, image_numpy in visuals.items():
                # print(f'label is {label}, shape is {image_numpy.shape}')
                ...
                if image_numpy.shape[0]==1 or image_numpy.shape[0]==2:
                    image_numpy = image_numpy[0,:,:]
        ...
        if self.use_html: # save images to a html file
            for label, image_numpy in visuals.items():
                print(label)#input_label, synthesized data
    ...
    def plot_current_errors(self, errors, step):
        if self.tf_log:
            for tag, value in errors.items():
                ...
                #     self.tf.summary.scalar(tag, value.detach().cpu().numpy(),step=step)
                #     self.writer.flush()
