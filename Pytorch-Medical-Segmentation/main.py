    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if hp.mode == '2d':
                ...
                y[y!=0] = 1 #not clear.
                #print(y.max())
                # print(y.shape)#[4,1,128,128]
                # img = transforms.ToPILImage()(y[0,0,:,:])
                # img.save('results/tmp/test_1.jpg')
            outputs = model(x)
...
def test():
    ...
    for i,subj in enumerate(test_dataset.subjects):
        ...
        if (hp.in_class == 1) and (hp.out_class == 1) :
            label_image = torchio.ScalarImage(tensor=output_tensor.numpy().astype(np.uint8), affine=affine)
            label_image.save(os.path.join(output_dir_test,f"{i:04d}-result_float"+hp.save_arch))

            # f"{str(i):04d}-result_float.mhd"

            output_image = torchio.ScalarImage(tensor=output_tensor_1.numpy().astype(np.uint8), affine=affine)
