#!/usr/bin/env python 
# -*- coding:utf-8 -*-
# Adapted from Resource (https://github.com/hristina-uzunova/TumorMassEffect) to train on masks
import os
import argparse
import numpy as np
...
from net_utils import warp_image_diffeomorphic, diffusion_regularise_displacement, GaussianLayer, sparsity_regularise_displacement
from img_utils import save3DImage, showTrainingData, showTestData, show3SlicesFromNumpy
...
split = int(np.floor(opt.valPr * num_train)) # since limited training samples, only 1 mask is picked as val
np.random.seed(32)
...
# showTestData(test_data_loader)

print('===> Building model')

with torch.cuda.device(opt.device):
    netG = ProbabilisticUnet(input_channels=4, dim=3, num_filters=[32, 64, 128],
                             latent_dim=15, no_convs_fcomb=3, beta=1)  # input: 4(BG, Ven, GM+WM, Tumor)xLxWxH
...
def test():
    print('===> Testing...')
    TestOutDir = os.path.join(outputDir, 'Test')
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_data_loader):
            images_def = batch['image_def'].to(device)
            tumor = batch['tumor'].to(device)
            image_def_original = batch['image_def_original']
            # generated_tumor = batch['generated_tumor'].squeeze(0)
            netG(torch.cat((images_def, tumor), dim=1), None, training=False)
            displ = netG.sample()
            displ = displ.permute(0, 2, 3, 4, 1)
            prediction, computed_displ = warp_image_diffeomorphic(images_def, displ, mode='bilinear',
                                                                  ret_displ=True)
            prediction = torch.argmax(prediction[:, :, :, :, :], dim=1)
            save3DImage(prediction.data[0, :, :, :].cpu().numpy().astype('uint8'),
                        outputDir + "/Test/prediction_" + str(i) + ".nii")
            show3SlicesFromNumpy(image_def_original.numpy().squeeze(axis=0), 'Deformed Image')
            show3SlicesFromNumpy(prediction.data[0, :, :, :].cpu().numpy(), 'Learned Undeformed')
