...
def diffeomorphic_3d(displacement, grid, scaling=-1):
    ...
    for i in range(scaling):
        ...
        # print(f'=======the size of displacement_trans is {displacement_trans.shape}, the size of grid is {grid.shape}========')
        displacement = displacement + F.grid_sample(displacement, displacement_trans + grid, padding_mode='border', mode='bilinear')
#=======the size of displacement_trans is torch.Size([2, 24, 100, 100, 4]), the size of grid is torch.Size([2, 24, 100, 100, 3])========
    return displacement.permute(0,2,3,4,1)

def inverse_diffeomorphic_3d(displacement, grid, scaling=-1):
    if scaling < 0:
        scaling = _compute_scaling_value(displacement)
    displacement = displacement / (2 ** scaling)
    print(f'the displacement is {displacement.shape}')
    displacement = displacement.permute(0,4,1,2,3)
    for i in range(scaling):
        displacement_trans = displacement.permute(0,2,3,4,1)
        displacement = displacement + F.grid_sample(displacement, grid - displacement_trans, padding_mode='border', mode='bilinear')
    return displacement.permute(0,2,3,4,1)
...
def warp_image_diffeomorphic(image, displacement,mode='bilinear',ret_displ=False):
    ...
    grid=torch.cat(grid_list,dim=0)
    # warp image
    print(f'======the dimension of displacement:{displacement.shape},the dimension of grid:{grid.shape}=========')
...
def inverse_warp_image_diffeomorphic(image, displacement,mode='bilinear',ret_displ=False):
    #displacement is defined by velocity

    image_size = image.shape
    # image size [N,C,W,H (D)]
    image_size=image_size[2:]#舍弃了前两维，batch+channel
    ...
    grid=torch.cat(grid_list,dim=0)
    # warp image
    displacement=inverse_diffeomorphic_3d(displacement, grid, scaling=-1)
    warped_image = F.grid_sample(image,grid-displacement,mode=mode)
    if ret_displ:
        return warped_image, displacement
    else:
        return warped_image
...
def compute_grid(image_size, dtype=torch.float32, device='cpu'):
    ...
    if(dim == 2):
        ...
        print(f'===========image size is {image_size},grid size is {grid.shape}==========')
